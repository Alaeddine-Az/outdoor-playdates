...
const PlaymateFinderPreview = () => {
  ...
  return <section ref={ref as React.RefObject<HTMLDivElement>} className="py-24 px-6 relative overflow-hidden bg-muted/30">
        ...
        <div className="flex flex-col lg:flex-row items-center gap-16">
          ...
          <div className={cn('max-w-xl', isIntersecting ? 'animate-fade-in' : 'opacity-0')}>
            ...
            <div className="space-y-6">
              <FeaturePoint icon={<Search className="h-5 w-5 text-primary" />} title="Intelligent Matching" description="Our AI suggests potential playmates based on location, interests, and age compatibility." />
              <FeaturePoint icon={<Shield className="h-5 w-5 text-primary" />} title="Verified Parents Only" description="All parents must complete our verification process before they can connect with others." />
              <FeaturePoint icon={<MapPin className="h-5 w-5 text-primary" />} title="Location-Based Discovery" description="Find families within walking distance or near your favorite play spots." />
            </div>
            <div className="mt-8">
          ...
          <div className={cn('w-full max-w-xl', isIntersecting ? 'animate-slide-in-right' : 'opacity-0')}>
            <div className="bg-white rounded-2xl shadow-soft border border-muted p-6">
              ...
              <div className="space-y-3">
                <PlaymateCard parentName="Sarah J." parentInitial="SJ" children={[{
                name: 'Oliver',
                age: 6
              }, {
                name: 'Sophia',
                age: 4
              }]} location="0.5 miles away" interests={['Nature Explorers', 'LEGO Building']} verified={true} />
                <PlaymateCard parentName="David B." parentInitial="DB" children={[{
                name: 'Emma',
                age: 5
              }]} location="0.8 miles away" interests={['Arts & Crafts', 'Sports']} verified={true} />
                <PlaymateCard parentName="Jessica & Mike T." parentInitial="JT" children={[{
                name: 'Liam',
                age: 7
              }, {
                name: 'Ava',
                age: 5
              }]} location="1.2 miles away" interests={['STEM Activities', 'Outdoor Games']} verified={true} />
              </div>
...
const PlaymateCard = ({
  ...
  interests,
  verified
}: PlaymateCardProps) => <div className="bg-muted/50 hover:bg-muted/80 transition-colors rounded-xl p-4">
      ...
      <div className="flex items-center">
        ...
        <div>
          <div className="flex items-center flex-wrap gap-1">
            ...
            {verified && <div className="flex items-center text-play-green text-xs">
    ...
    <div className="ml-0 sm:ml-12 mt-1 space-y-1">
      <div className="text-sm text-muted-foreground">
        {children.map((child, index) => <span key={index}>
      ...
      <div className="flex flex-wrap gap-2 mt-2">
        {interests.map((interest, index) => <span key={index} className="inline-block px-2 py-0.5 bg-white text-xs rounded-full border border-muted">
            {interest}
          </span>)}
      </div>
    </div>
